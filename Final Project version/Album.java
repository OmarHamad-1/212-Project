...
// helper function for the getPhotos()
	...
	// for testing
	public void display() {
		Linkedlist<Photo> AlbumPhotos = getPhotos();
		System.out.println("all "+this.name+" photos");
		if (AlbumPhotos.empty())
			System.out.println(this.name+" is Empty");
		
		AlbumPhotos.findFirst();
		while(!AlbumPhotos.last()) {
			System.out.println(AlbumPhotos.retrieve().getPath());
			AlbumPhotos.findNext();
		}
		System.out.println(AlbumPhotos.retrieve().getPath());
	}
